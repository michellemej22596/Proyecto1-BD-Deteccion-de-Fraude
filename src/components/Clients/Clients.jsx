...
const Update = () => {
  ...
  const [updatedData, setUpdatedData] = useState({
    ...
    edad: '',
    pais: '',
    estado: '',  // Agregamos el campo para el estado
  });
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setUpdatedData({
          ...
          estado: response.data.estado, // Establecemos el estado del cliente
        });
      }
    } catch (error) {
  ...
  const handleChangeStatus = async () => {
    ...
    try {
      const response = await api.patch(`/clientes/${clientIdentifier}/estado`, {
        estado: updatedData.estado, // Mandamos el estado a actualizar
      });
      if (response.status === 200) {
        setSuccess(true);
      }
    } catch (error) {
      setError('Hubo un error al cambiar el estado del cliente.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg">
      ...
      {success && <div className="bg-green-500 text-white p-2 rounded mb-4">Operación exitosa!</div>}
      ...
      <form onSubmit={handleSubmit}>
        ...
        {clientData && (
          <div className="mt-4">
            ...
            {/* Agregamos un campo para el estado */}
            <div className="mb-4">
