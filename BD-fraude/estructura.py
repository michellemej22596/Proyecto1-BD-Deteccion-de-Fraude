...
# 🔹 Buscar el ID más alto en cada tipo de nodo y guardarlo
queries_max_id = {
...
# Obtener el ID más alto de cada tipo
max_ids = {key: query(value)[0]["max_id"] for key, value in queries_max_id.items()}

# 🔹 Crear restricciones (para evitar duplicados)
estructura_nodos = [
...
# 🔹 Insertar nodos con ID incremental
estructura_insertar_nodos = [
    f"CREATE (:Cliente {{Cliente_ID: {max_ids['Cliente'] + 1}, Nombre: 'Nuevo Cliente'}})",
    f"CREATE (:CuentaBancaria {{Cuenta_ID: {max_ids['CuentaBancaria'] + 1}, Tipo: 'Corriente'}})",
    f"CREATE (:Transaccion {{Transaccion_ID: {max_ids['Transaccion'] + 1}, Monto: 1000.0}})",
    f"CREATE (:Comercio {{Comercio_ID: {max_ids['Comercio'] + 1}, Nombre: 'Nuevo Comercio'}})",
    f"CREATE (:Ubicacion {{Ubicacion_ID: {max_ids['Ubicacion'] + 1}, Ciudad: 'Guatemala'}})",
    f"CREATE (:Tarjeta {{Tarjeta_ID: {max_ids['Tarjeta'] + 1}, Tipo: 'Crédito'}})"
]

# 🔹 Crear relaciones si los datos ya existen
estructura_relaciones = [
    ...
    "MATCH (c:Cliente), (com:Comercio) WHERE c.Cliente_ID = com.Cliente_ID MERGE (c)-[:OPERA_EN]->(com)",
